...
class SetTopicValue(Example):
    async def run(
        ...
        async with sessions().principal(principal).credentials(
            ...
            constraint = ConstraintFactory().value(diffusion.datatypes.JSON(json_data))
            await session.topics.set_topic(
                topic, diffusion.datatypes.JSON(json_data), topic_specification
            )

            json_data2 = {"diffusion": ["data", "more data", "yet more data"]}
            await session.topics.set_topic(
                topic, diffusion.datatypes.JSON(json_data2), topic_specification, constraint
            )
