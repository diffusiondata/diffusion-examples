/**
 * Copyright © 2018 - 2023 DiffusionData Ltd.
 *
...
/**
 * This example shows how to add, subscribe and update a Double topic
 */
        ...
        void *context)
{
        if(old_value) {
                ...
                if(!read_diffusion_double_value(old_value, &old_double_value, &old_value_error)) {
                        fprintf(
                                stderr,
                                "Error parsing double old value: %s\n",
                                get_diffusion_api_error_description(old_value_error));
                        diffusion_api_error_free(old_value_error);
                        return HANDLER_SUCCESS;
                }

                printf("Old double value: %f\n", old_double_value);
        }

        DIFFUSION_API_ERROR new_value_error;
        double new_double_value;
        if(!read_diffusion_double_value(new_value, &new_double_value, &new_value_error)) {
                fprintf(
                        stderr,
                        "Error parsing double new value: %s\n",
                        get_diffusion_api_error_description(new_value_error));
                diffusion_api_error_free(new_value_error);
                return HANDLER_SUCCESS;
        }

        printf("New double value: %f\n\n", new_double_value);
        return HANDLER_SUCCESS;
}
...
static void dispatch_double_update(
        SESSION_T *session,
        const char *topic_path)
{
        ...
        if(!write_diffusion_double_value(value, buf)) {
                fprintf(stderr, "Unable to write the double update\n");
                buf_free(buf);
                return;
        }
...
int main(int argc, char** argv)
{
        ...
        const char *topic_path = "double-example";
        ...
        // Add the double topic
        TOPIC_SPECIFICATION_T *specification = topic_specification_init(TOPIC_TYPE_DOUBLE);
        ...
        // Set up and add the value stream to receive double topic updates
