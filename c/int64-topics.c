...
static void
dispatch_int64_update(SESSION_T *session, CONVERSATION_ID_T *updater_id, const char *topic_path)
{
        int64_t value = rand();
        ...
        apr_thread_mutex_lock(mutex_value_stream);
        update_value_with_datatype(session, DATATYPE_INT64, update_value_params);
...
int main(int argc, char** argv)
{
        ...
        for(int i = 1; i <= 120; i++) {
                dispatch_int64_update(session, updater_id, topic_path);
