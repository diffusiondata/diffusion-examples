...
static void
dispatch_string_update(SESSION_T *session, CONVERSATION_ID_T *updater_id, const char *topic_path, int update_number)
{
        ...
        apr_thread_mutex_lock(mutex_value_stream);
        update_value_with_datatype(session, DATATYPE_STRING, update_value_params);
...
int main(int argc, char** argv)
{
        ...
        for(int i = 1; i <= 120; i++) {
                dispatch_string_update(session, updater_id, topic_path, i);
