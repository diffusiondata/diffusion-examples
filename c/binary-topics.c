...
/**
 * This example shows how to add, subscribe and update a Binary topic
 */
             ...
             void *context)
{
        if(old_value) {
                ...
                void *old_binary_value;
                if(!read_diffusion_binary_value(old_value, &old_binary_value, &old_value_error)) {
                        fprintf(stderr, "Error parsing binary old value: %s\n", get_diffusion_api_error_description(old_value_error));
                        diffusion_api_error_free(old_value_error);
                        return HANDLER_SUCCESS;
                }

                printf("Old binary value: %s\n", (char *)old_binary_value);
                free(old_binary_value);
        }

        DIFFUSION_API_ERROR new_value_error;
        void *new_binary_value;
        if(!read_diffusion_binary_value(new_value, &new_binary_value, &new_value_error)) {
                fprintf(stderr, "Error parsing binary new value: %s\n", get_diffusion_api_error_description(new_value_error));
...
static void
dispatch_binary_update(SESSION_T *session, CONVERSATION_ID_T *updater_id, const char *topic_path)
{
        ...
        if(!write_diffusion_binary_value(random_bytes, buf, sizeof(random_bytes))) {
                fprintf(stderr, "Unable to write the binary update\n");
                buf_free(buf);
                return;
        }
        ...
        apr_thread_mutex_lock(mutex_value_stream);
        update_value_with_datatype(session, DATATYPE_BINARY, update_value_params);
...
int main(int argc, char** argv)
{
        ...
        for(int i = 1; i <= 120; i++) {
                dispatch_binary_update(session, updater_id, topic_path);
