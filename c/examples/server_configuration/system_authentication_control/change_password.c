    ...
    CREDENTIALS_T *credentials)
{
    ...
    SCRIPT_T *change_password_script = script_create();
    update_auth_store_set_password(
        change_password_script, "super_user", "hunter2"
    );

    const UPDATE_SYSTEM_AUTHENTICATION_STORE_PARAMS_T change_password_params = {
        .on_update = on_system_authentication_store_updated,
        .on_error = on_error,
        .update_script = change_password_script
    };

    update_system_authentication_store(session, change_password_params);
    MUTEX_WAIT
    script_free(change_password_script);
