    ...
    CREDENTIALS_T *credentials)
{
    ...
    SET_T *set_global_permissions = set_new_int(2);
    set_add(set_global_permissions, &SECURITY_GLOBAL_PERMISSIONS_TABLE[GLOBAL_PERMISSION_VIEW_SERVER]);
    set_add(set_global_permissions, &SECURITY_GLOBAL_PERMISSIONS_TABLE[GLOBAL_PERMISSION_VIEW_SESSION]);

    SCRIPT_T *script = script_create();
    update_security_store_global_role_permissions(
        script, "CLIENT", set_global_permissions
    );
    ...
    printf(
        "\nAdding the following permissions to the global permisions of Role CLIENT: "
        "VIEW_SERVER and VIEW_SESSION.\n"
    );
    ...
    MUTEX_WAIT
    script_free(script);
    set_free(set_global_permissions);
