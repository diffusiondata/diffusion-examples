    ...
    CREDENTIALS_T *credentials)
{
    ...
    printf("Original Security Store Settings\n");
    utils_print_security_store(session, "");

    LIST_T *list_anon_roles = list_create();
    list_append_last(list_anon_roles, "CLIENT");
    list_append_last(list_anon_roles, "AUTHENTICATION_HANDLER");

    SCRIPT_T *script = script_create();
    update_security_store_anonymous_session_roles(
        script, list_anon_roles
    );
    ...
    MUTEX_WAIT
    script_free(script);
    list_free(list_anon_roles, NULL);
