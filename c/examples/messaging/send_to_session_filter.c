...
static int on_number_sent(
    int number_sent,
    void *context)
{
    printf("Message sent to %d session(s).\n", number_sent);
    return HANDLER_SUCCESS;
}
    ...
    CREDENTIALS_T *credentials)
{
    ...
    SESSION_T *receiving_session = session_create(
        url, "admin", credentials, NULL, NULL, NULL
    );
    ...
    SESSION_T *non_receiving_session = session_create(
        url, "control", credentials, NULL, NULL, NULL
    );
    ...
    SEND_REQUEST_TO_FILTER_PARAMS_T send_request_params = {
        .path = message_path,
        .filter = "$Principal EQ 'admin'",
