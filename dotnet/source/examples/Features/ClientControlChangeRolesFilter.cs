...
namespace PushTechnology.ClientInterface.Example.Features {
    /// <summary>
    /// Client implementation that changes the assigned roles of a session using a filter.
    /// </summary>
    public sealed class ClientControlChangeRolesFilter : IExample {
        /// <summary>
        /// Runs the client control example that changes the assigned roles of a session using a filter.
        /// </summary>
        ...
        public async Task Run( CancellationToken cancellationToken, string[] args ) {
            ...
            try
            {
                WriteLine("Removing the Administrator role for the session with a filter.");

                await clientControl.ChangeRolesAsync("$SessionId is \"" + adminSession.SessionId + "\"", new Collection<string>() { "ADMINISTRATOR" }, new Collection<string>());
            }
            catch (Exception ex)
            {
