...
{
     public sealed class SubscribeWithCrossCompatibleValueStream : Example
     {
        ...
        private sealed class JSONStream : IValueStream<IJSON>
        {
            ...
            {
                WriteLine($"JSON stream subscribed to {topicPath}.");
            }

            public void OnUnsubscription(string topicPath, ITopicSpecification specification, TopicUnsubscribeReason reason)
            {
                WriteLine($"JSON stream unsubscribed from {topicPath}: {reason}.");
            }

            public void OnValue(string topicPath, ITopicSpecification specification, IJSON oldValue, IJSON newValue)
            {
                WriteLine($"JSON stream {topicPath} changed from {(oldValue == null ? "NULL" : oldValue.ToJSONString())} to {newValue.ToJSONString()}.");
            }
        }
        ...
        private sealed class StringStream : IValueStream<string>
        {
            ...
            public void OnSubscription(string topicPath, ITopicSpecification specification)
            {
                WriteLine($"String stream subscribed to {topicPath}.");
            }

            public void OnUnsubscription(string topicPath, ITopicSpecification specification, TopicUnsubscribeReason reason)
            {
                WriteLine($"String stream unsubscribed from {topicPath}: {reason}.");
            }

            public void OnValue(string topicPath, ITopicSpecification specification, string oldValue, string newValue)
            {
                WriteLine($"String stream {topicPath} changed from {oldValue ?? "NULL"} to {newValue}.");
            }
        }
    }
}
