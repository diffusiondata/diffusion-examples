...
namespace PushTechnology.ClientInterface.Examples.PubSub.JSONPatch
{
    public sealed class JSONPatchMove : Example
    {
        public override async Task Run(CancellationToken cancellationToken, string[] args)
        {
            ...
            string json = "{\"Fred\":\"Flintstone\",\"Barney\":\"Rubble\",\"George\":\"Jetson\"}";
            ...
            session.Topics.AddStream(topic, jsonStream);

            await session.Topics.SubscribeAsync(topic, cancellationToken);

            var result = await session.TopicUpdate.ApplyJSONPatchAsync(topic, "[{\"op\":\"move\", \"from\": \"/Fred\", \"path\": \"/Meet The Flintstones~1Fred\"}]", cancellationToken);

            WriteLine($"JSON patch move has been successfully applied.");
