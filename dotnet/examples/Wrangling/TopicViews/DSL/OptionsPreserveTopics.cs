...
{
    public sealed class OptionsPreserveTopics : Example
    {
        public override async Task Run(CancellationToken cancellationToken, string[] args)
        {
            ...
            string json = "{\"name\":\"Fred Flintstone\"}";
            ...
            var view1 = await session.TopicViews.CreateTopicViewAsync("topic_view_1", "map my/topic/path to views/preserved/<scalar(/name)> preserve topics", cancellationToken);
            WriteLine($"Topic View {view1.Name} has been created.");

            var view2 = await session.TopicViews.CreateTopicViewAsync("topic_view_2", "map my/topic/path to views/not_preserved/<scalar(/name)>", cancellationToken);
            WriteLine($"Topic View {view2.Name} has been created.");

            await Task.Delay(2000);

            string json2 = "{\"name\":\"Wilma Flintstone\"}";
            await session.TopicUpdate.SetAsync(topic, Diffusion.DataTypes.JSON.FromJSONString(json2), cancellationToken);

            await Task.Delay(2000);

            string json3 = "{\"name\":\"Pebbles Flintstone\"}";
            await session.TopicUpdate.SetAsync(topic, Diffusion.DataTypes.JSON.FromJSONString(json3), cancellationToken);
