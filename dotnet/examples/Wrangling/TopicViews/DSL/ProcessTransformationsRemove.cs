...
{
    public sealed class ProcessTransformationsRemove : Example
    {
        public override async Task Run(CancellationToken cancellationToken, string[] args)
        {
            ...
            string topic = "my/topic/path/1";
            string topic2 = "my/topic/path/2";
            string topicSelector = "?views//";

            var topicSpecification = session.TopicControl.NewSpecification(TopicType.JSON);

            string json = "{\"name\":\"APPL\",\"quantity\":100,\"price_per_share\":12.34}";
            ...
            string json2 = "{\"name\":\"AMZN\",\"quantity\":256,\"price_per_share\":87.65}";
            await session.TopicUpdate.AddAndSetAsync(topic2, topicSpecification, Diffusion.DataTypes.JSON.FromJSONString(json2), cancellationToken);
            ...
            var view1 = await session.TopicViews.CreateTopicViewAsync("topic_view_1", "map ?my/topic/path// to views/<path(3)> process {set(/total, calc '/quantity * /price_per_share');  remove(/quantity);  remove(/price_per_share)}", cancellationToken);
