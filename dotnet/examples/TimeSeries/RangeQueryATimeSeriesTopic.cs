...
{
    public sealed class RangeQueryATimeSeriesTopic : Example 
    {
        public override async Task Run( CancellationToken cancellationToken, string[] args ) 
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                ...
                await session.TimeSeries.AppendAsync<double?>(topic, newValue, cancellationToken);
            }

            await session.TimeSeries.EditAsync<double?>(topic, 10, 3.14, cancellationToken);

            // Create a time series range query from values 5 to 15.
            var rangeQuery = session.TimeSeries.RangeQuery.As<double?>();

            var queryResult = await rangeQuery
                .From(5)
                .To(15)
                .SelectFromAsync(topic, cancellationToken);

            var eventResults = queryResult.Events.ToList();
