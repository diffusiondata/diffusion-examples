...
{
    public sealed class AssignRoles : Example
    {
        public override async Task Run(CancellationToken cancellationToken, string[] args)
        {
            ...
            string updateScript = session.SystemAuthenticationControl.Script
                .AddPrincipal("super_user", "password12345", new List<string>() { "CLIENT " })
                .ToScript();

            await session.SystemAuthenticationControl.UpdateStoreAsync(updateScript, cancellationToken);

            WriteLine($"Adding ADMINISTRATOR role to super_user principal.");

            updateScript = session.SystemAuthenticationControl.Script
                .AssignRoles("super_user", new[] { "ADMINISTRATOR" }).ToScript();
