...
namespace Examples {
    ...
    public class ControlClientIdentityChecks {
        ...
        public ControlClientIdentityChecks() {
            ...
            authenticationControl.SetAuthenticationHandler( "example-handler",
                Enum.GetValues( typeof( DetailType ) ).OfType<DetailType>().ToList(), new Handler() );
        }
        ...
        private class ServerHandlerDefault : IServerHandler {
            ...
            ///
            /// A session can register at most one a single handler of each type. If there is already a handler
            /// registered for the topic path the operation will fail, the registered handler will be closed, and the
            /// session error handler will be notified. To change the handler, first close the previous handler.
            /// </summary>
