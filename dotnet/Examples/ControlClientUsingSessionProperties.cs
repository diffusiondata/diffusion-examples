...
namespace Examples {
    ...
    public class ControlClientUsingSessionProperties {
        ...
        private class PropertiesListener : SessionPropertiesListenerDefault {
            ...
            /// When the listener is registered, this will be called for all existing sessions. It will then be called
            /// for every new session that opens whilst the listener is registered.
            /// This will be called for every client session regardless of requested session properties.
            /// </summary>
            /// <param name="sessionId">The session identifier.</param><param name="properties">The map of requested
            /// session property values. This can be empty if no properties were requested. If a requested property did
            /// not exist then it will not be prsent in the map.</param>
            public override void OnSessionOpen( SessionId sessionId, IDictionary<string, string> properties ) {
                if ( "Accounts".Equals( properties[ "Department" ] ) && "IT".Equals( properties[ "$Country" ] ) ) {
