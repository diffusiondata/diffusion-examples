...
diffusion.connect({
    ...
    const table = diffusion.newBranchMappingTableBuilder()
    ...
    {
        const branchMappingTable = await session.sessionTrees.getBranchMappingTable(sessionTreeBranch);

        for (var branchMapping of branchMappingTable.getBranchMappings())
        {
