...
-(void)startWithURL:(NSURL*)url {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // Enabling queue conflation to sessions that have UK as their country
        ...
        [self->_controlSession.clientControl setQueueConflation:YES
                                              ...
                                              completionHandler:^(NSInteger matchedSessions, NSError * _Nullable error)
        {
            if (error != nil) {
                NSLog(@"An error occured while enabling queue conflation using session filter: [%@]", error);
            }
            else {
                NSLog(@"Total clients with queue conflation enabled by session filter: %ld", (long) matchedSessions);
            }
        }];
    });
}
