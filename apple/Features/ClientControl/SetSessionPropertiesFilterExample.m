...
-(void)startWithURL:(NSURL*)url {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // Setting country of normal session 1 to UK via sessionID
        ...
        // Setting country of normal session 2 to US via sessionID
        ...
        // Adding a new session property to session whose country is UK
        NSString *sessionFilter = [NSString stringWithFormat:@"%@ EQ 'UK'", PTDiffusionSession.countryPropertyKey];
        NSDictionary *newSessionProperties = @{@"new_property_1" : @"hello world!"};
        [self->_controlSession.clientControl setSessionProperties:newSessionProperties
                                                      usingFilter:sessionFilter
                                                completionHandler:^(NSInteger matches, NSError * _Nullable error)
        {
            if (error != nil) {
                NSLog(@"An error occured while setting session properties using a session filter: [%@]", error);
            }
            else {
                NSLog(@"Total clients affected by session properties change: %ld", (long) matches);
            }
        }];
    });
}
