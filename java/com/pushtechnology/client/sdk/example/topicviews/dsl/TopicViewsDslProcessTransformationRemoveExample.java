...
/**
 * This example demonstrates how to use the process transformation to remove fields
 * from a JSON topic in a topic view.
 * <P>
 * A topic view is created that calculates a new field `/total` by multiplying
 * `/quantity` and `/price_per_share`, then removes the `/quantity` and `/price_per_share`
 * fields from the JSON structure.
 *
 * @author DiffusionData Limited
 */
public class TopicViewsDslProcessTransformationRemoveExample {

    private static final Logger LOG =
        LoggerFactory.getLogger(TopicViewsDslProcessTransformationRemoveExample.class);
