...
public class RemoveSingleTopicViaPathExample {
    ...
    public static void main(String[] args)
        ...
        try (Session session = Diffusion.sessions()
            ...
            final String topicPath = "my/topic/path/to/be/removed";

            final TopicControl.AddTopicResult addResult =
                topicControl.addTopic(
                        topicPath, topicSpecification)
                    .join();

            if (addResult == TopicControl.AddTopicResult.CREATED) {
            ...
            if (removedCount == 1) {
                LOG.info("Topic has been removed.");
            }
            else {
                final String message = format(
                    "Topic: '%s' was expected to be removed, but %d topics were removed.",
                    topicPath, removedCount);
