...
export async function trustClientProposedPropertyIn(): Promise<void> {
    /// tag::system_authentication_control_trust_client_proposed_property_in[]
    ...
    const properties2 = await session1.clients.getSessionProperties(
        session2.sessionId,
        clients.PropertyKeys.ALL_PROPERTIES
    );
    ...
    /// tag::log
    expect(properties3['Flintstone']).toBe('Fred');
    /// end::log
    ...
    /// end::system_authentication_control_trust_client_proposed_property_in[]
}
