...
/// tag::log
import { expectTopicToHaveProperties, PartiallyOrderedCheckpointTester } from '../../../../../test/util';
/// end::log

export async function topicViewsDslOptionsTopicPropertyMapping(): Promise<void> {
    /// tag::log
    const check = new PartiallyOrderedCheckpointTester([[
        'Subscribed to views/my/topic/path'
    ]]);
    /// end::log
    /// tag::topic_views_dsl_options_topic_property_mapping[]
    ...
    const topicView = await session.topicViews.createTopicView(
        ...
        `map ?my/topic/path// to views/<path(0)>
            with properties 'CONFLATION':'off', 'COMPRESSION':'false', 'DONT_RETAIN_VALUE':'true'`
    );
    console.log(`Topic View ${topicView.name} has been created.`);

    await session.closeSession();
    /// end::topic_views_dsl_options_topic_property_mapping[]
    /// tag::log
    await expectTopicToHaveProperties('views/my/topic/path', {
        CONFLATION: 'off',
        COMPRESSION: 'false',
