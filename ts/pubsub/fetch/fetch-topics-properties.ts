...
export async function pubSubFetchTopicProperties(): Promise<void> {
    ...
    const specification1 = new topics.TopicSpecification(topics.TopicType.JSON, {
        'DONT_RETAIN_VALUE': 'true',
        'PERSISTENT': 'false',
        'PUBLISH_VALUES_ONLY': 'true'
    });
    for (let count = 0; count < 5; count++) {
        const jsonData = datatypes.json().from({ diffusion: `data #${count}` });
    ...
    const specification2 = new topics.TopicSpecification(topics.TopicType.STRING);
    for (let count = 0; count < 5; count++) {
        await session.topicUpdate.set(
    ...
    const fetchResult1 = await session.fetchRequest()
        .withProperties()
        .topicTypes([ topics.TopicType.JSON ])
        .fetch('?my/topic/path//');
    ...
    const fetchResult2 = await session.fetchRequest()
        .withProperties()
        .topicTypes([ topics.TopicType.STRING ])
        .fetch('?my/topic/path//');
